        ...
        <div class="row border-0 p-2">
          <div class="col">
            ...
            <div class="d-flex">
              ...
              <h6
                class="ml-1 mr-auto"
                style="font-size:10px;width:450px;overflow:hidden;text-overflow:ellipsis"
              >{{transaction.item.remark}}</h6>
              <h6
                class="text-secondary"
                style="font-size:10px"
              >{{transaction.item.dateName}} {{transaction.item.date | date }}</h6>
            </div>
          </div>
        </div>
      </template>
...
export default {
  ...
  async created () {
    ...
    await this.refreshTransactions(selectedPurchase)
    this.$store.watch(
      state => {
        return this.$store.getters.selected('Purchase')
      },
      async (newValue, oldValue) => {
        await this.refreshTransactions(newValue)  
      }
    )
  },
  methods: {
    ...
    async refreshTransactions (po) {
      const data = await this.$store.dispatch('getPoTransactions', po.links.transactions)
