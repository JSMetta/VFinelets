<template>
  <master-details  :master="master">
    <tabpage class="col-9" :tabs="tabs" :activated="currentTab" slot="details" @changed="onPageChanged">
      <div slot="overview">
        <h1>Welcome to our support DEV center 1.0.1</h1>
        <p>
...
export default {
  ...
  methods: {
    ...
    async onRemove (obj) {
      const res = await state.dispatch('removeProgram', obj.links.self)
      if (res.ok) {
    ...
    async onSearchPrograms (cond) {
      ...
      let data = await state.dispatch('searchPrograms', r)
      this.programs = data
    },
    selectProgram (data) {
      state.commit('selectedProgram', data)
      let path = {
        name: 'runProgram'
      }
      this.$router.push(path)
